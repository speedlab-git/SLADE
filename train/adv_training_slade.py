...
parser.add_argument('--lambda_weight', type=float, default=0.5, help='Weight for combining patch and global losses')
parser.add_argument('--patch_size', type=int, default=32, help='Size of the patches to be extracted from images')
...
def extract_patches(image, img_size=32, stride=None):
    """Extract patches with padding to maintain CLIP's expected input size (224x224)"""
    if stride is None:
        stride = stride

    # Pad the image to 224x224 if smaller
    b, c, h, w = image.shape
    pad_h = max(0, 224 - h)
    pad_w = max(0, 224 - w)
    ...
    patches = F.unfold(image, kernel_size=224, stride=stride)
    patches = patches.transpose(1, 2).contiguous().view(-1, c, 224, 224)
    return patches

# Symmetric contrastive loss with stop-gradient
def symmetric_contrastive_loss(embedding_clean, embedding_adv):
    loss1 = -F.cosine_similarity(embedding_clean, embedding_adv.detach(), dim=-1).mean()
    loss2 = -F.cosine_similarity(embedding_adv, embedding_clean.detach(), dim=-1).mean()
...
## This portion for getting the text embeddings-> Not used in adv training
#=========================================================================
    with torch.no_grad():
...
def train_one_epoch(
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        # loss_total = args.lambda_weight * patch_loss + (1 - args.lambda_weight) * global_loss
        stride = args.patch_size  # Use patch_size as stride for overlap
        clean_patches = extract_patches(data, img_size=224, stride=stride)
        adv_patches = extract_patches(data_adv, img_size=224, stride=stride)

        print("patches generated")
   
        clean_patch_embeddings = model(clean_patches, output_normalize=args.output_normalize)
        adv_patch_embeddings = model(adv_patches, output_normalize=args.output_normalize)
     

        del clean_patches, adv_patches
        ...
        patch_loss = symmetric_contrastive_loss(clean_patch_embeddings, adv_patch_embeddings)

        global_loss = symmetric_contrastive_loss(embedding_clean, embedding_adv)
        loss_total = args.lambda_weight * patch_loss + (1 - args.lambda_weight) * global_loss

        loss=loss_total
        print("total loss for slade:====", loss_total)
        loss_total.backward()
        optimizer.step()
        optimizer.zero_grad()
        step_total += 1
